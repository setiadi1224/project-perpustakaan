...
<style>
    ...
    .status.ditolak { background: rgba(107,114,128,.15); color:#9ca3af; }
    .status.konfirmasi { background: rgba(168,85,247,.15); color:#c084fc; }
                        ...
                        <td>
                            @if ($item->status == 'dipinjam' && $item->status_pengembalian != 'menunggu')
                                <form action="{{ route('user.return', $item->id) }}" method="POST">
                                    @csrf
                                    <button class="btn-return" onclick="return confirm('Ajukan pengembalian buku ini?')">
                                        Kembalikan
                                    </button>
                                </form>
        ...
        <div class="card-list">
            @foreach ($riwayats as $item)
                ...
                <div class="card">
                    ...
                    <div class="card-item">📅 Pinjam: {{ $item->tanggal_pinjam ? \Carbon\Carbon::parse($item->tanggal_pinjam)->format('d M Y') : '-' }}</div>
                    <div class="card-item">📆 Kembali: {{ $item->tanggal_kembali ? \Carbon\Carbon::parse($item->tanggal_kembali)->format('d M Y') : '-' }}</div>

                    <div class="card-item">
                        Status:
                        @if ($item->status_pengembalian == 'menunggu')
                            <span class="status konfirmasi">Menunggu</span>
                        @elseif($item->status == 'dipinjam' && $terlambat)
                            <span class="status terlambat">Terlambat</span>
                        @elseif($item->status == 'dipinjam')
                            <span class="status dipinjam">Dipinjam</span>
